      ...
      >
        ...
        <div className="block w-full ">
          ...
          <table className="items-center md:w-auto bg-transparent border-collapse ta">
            ...
            <tbody>
              {props.commentsDetail && props.commentsDetail.map(blog => {
                return(
                  <tr>
                ...
                <td className="border-t-0 px-4 align-middle border-l-0 border-r-0 text-base whitespace-nowrap p-4">
                  {blog.isVerified ? <FcApprove size={30} /> : <FcDisapprove size={30}/>}
                </td>
                <td className="border-t-0 px-4 align-middle border-l-0 border-r-0 text-base whitespace-nowrap p-4">
                ...
                <td className="border-t-0  px-6 align-middle border-l-0 border-r-0 text-base whitespace-nowrap ">
                  {blog.isVerified ? 
                          <Button className="w-100" variant="danger" onClick={()=> {let flag=true; return props.handleApprove(blog.currentID,flag)}}>Disapprove</Button> 
                          : <Button className="w-100" onClick={()=>{let flag=false; return props.handleApprove(blog.currentID,flag)}} >Approve</Button> }
                </td>
                </tr>
                )
              })}
