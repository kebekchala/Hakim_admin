...
export default function CardTable(props,{ color }) {
  ...
  const [commentsDetail, setCommentsDetails] = useState([]);
    useEffect(() => {
        return app.firestore().collection("doctors").onSnapshot(querySnapshot => {
            const list = [];
            querySnapshot.forEach(doc => {
               const {fullName,bio,city,country,email,isVerified,uid,idPicUrl} = doc.data();
                list.push({
                    currentID: uid,
                    name:fullName,
                    bio,
  ...
  return (
    ...
    {
    props.val === "verify" ?
       <VerifyTable listHeader={listHeader} commentsDetail={commentsDetail} handleApprove={handleApprove} />
